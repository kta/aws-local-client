...
切り替え、同一スイートを LocalStack / floci / ministack / kumo の 4 種に対して実行する。
エミュレータごとの API 対応差は実機プローブによる capability ゲート
(`e2e/helpers/capabilities.ts`)で吸収し、どのエミュレータでも同一スイートが green になる。
...
| `kumo` | docker | `ghcr.io/sivchari/kumo:0.25.3` |
...
### iOS E2E(同一スイートを iOS シミュレータで実行)

同じ spec スイートを、Appium + XCUITest ドライバの webview コンテキスト経由で
Tauri iOS アプリ(WKWebView)に対して実行できる(デスクトップの埋め込み WebDriver
プラグインはモバイル未対応のため。詳細:
`docs/superpowers/specs/2026-07-22-ios-e2e-appium-design.md`)。要 Xcode +
rustup ターゲット `aarch64-apple-ios-sim`。

```bash
# 1) iOS シミュレータ向け E2E ビルド(署名不要)
npm run e2e:ios:build
# 2) エミュレータを起動して、iOS スイートを実行
scripts/emulator.sh start ministack && scripts/emulator.sh wait ministack
E2E_ENDPOINT=http://localhost:4566 npm run e2e:ios
```

シミュレータはホストとネットワークを共有するため、アプリ内の `localhost:<port>` は
ホストのエミュレータへ届く。デバイスは `IOS_DEVICE` で指定(既定: 利用可能な iPad を優先)。
...
- **`e2e.yml`(E2E)**: PR、main / develop への push、手動実行(`workflow_dispatch`)で発火。
   - `e2e-linux`: ubuntu 上で `localstack` / `floci` / `ministack` / `kumo` の 4 エミュレータを
    マトリクスで起動し、`xvfb-run` で全スイートを実行(エミュレータ互換性の検証。
    ネットワークプロトコルの話なので OS 非依存)。
  - `e2e-macos` / `e2e-windows`: pip 版 ministack を起動し、同一スイートを実行(OS 互換性の検証)。
  - `e2e-ios`: iOS シミュレータ上のアプリに対して同一スイートを Appium/XCUITest で実行。
  - いずれも実バイナリ(デスクトップは `npm run e2e:build`、iOS は `npm run e2e:ios:build`)を
    生成してから実行する。
