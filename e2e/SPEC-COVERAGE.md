...
| R42 | SNS トピックタグタブ: ListTagsForResource 表示・TagResource で追加(floci は Rust 側リカバリ)・UntagResource で削除 | sns.e2e.ts#R42: adds a topic tag (SDK verified), then removes it |
| R43 | S3 バケット詳細のタブ化 + プロパティ(バージョニングトグル・タグ編集・CORS/ポリシー JSON 保存) | s3.e2e.ts#R43: properties tab toggles versioning and saves tags/CORS/policy |
...
| R45 | S3 オブジェクトのコピー(別キーへ CopyObject)・フォルダ作成(`<prefix>/` 0 バイト put) | s3.e2e.ts#R45: copies an object and creates a folder |
...
| R47 | RDS ダッシュボード: インスタンス数 / available 数 / スナップショット数のサマリカード + 「データベースを作成」クイックアクション。describe 非対応なら rds-unsupported バナー | rds.e2e.ts#R47: shows summary cards on a describe-capable emulator / R47: shows the rds-unsupported banner on an unsupported emulator [^rds] |
| R48 | RDS インスタンス操作: 行アクションで停止 / 起動 / 再起動 / 変更(ApplyImmediately、storage 20→30 を SDK 検証)。操作 API 非対応環境(floci)では操作失敗を通常 ErrorBanner で表示 | rds.e2e.ts#R48: stops, starts, reboots and modifies an instance / R48: surfaces an error banner when an operation is rejected [^rds] |
| R49 | RDS スナップショット: UI 作成 → 復元(新インスタンスを SDK 検証)→ 確認付き削除で行消滅。describe 非対応なら snapshots-unsupported バナー | rds.e2e.ts#R49: creates, restores and deletes a snapshot / R49: shows the snapshots-unsupported banner on an unsupported emulator [^rds] |
| R50 | RDS パラメータグループ: UI 作成 → 一覧 → 行クリックでパラメータ表示。describe 非対応(localstack)なら parameter-groups-unsupported バナー | rds.e2e.ts#R50: creates a group, lists it and shows its parameters / R50: shows the parameter-groups-unsupported banner on localstack [^rds] |
...
R47〜R50(ダッシュボード / インスタンス操作 / スナップショット / パラメータグループ)も
同じ `branch` を再利用して能力分岐する。ライフサイクル系(R48・R49 の作成/復元/削除)は
ministack(create 分岐)でのみ実行し、他分岐は自己スキップ。R47・R49 の unsupported バナー節は
localstack(unsupported 分岐)で検証。R50 は describe 対応(ministack/floci の create・readonly
分岐)で CRUD、localstack で `parameter-groups-unsupported` バナーを検証する
(delete は spec R50 により失敗許容 = best-effort)。
