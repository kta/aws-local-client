...
| R48 | RDS インスタンス操作: 行アクションで停止 / 起動 / 再起動 / 変更(ApplyImmediately、storage 20→30 を SDK 検証)。操作 API 非対応環境では操作失敗を通常 ErrorBanner で表示 | rds.e2e.ts#R48: stops and starts an instance / R48: surfaces an error banner when stop/start is unsupported / R48: modifies an instance / R48: modify round-trips where the change is not applied / R48: reboots an instance without an error / R48: surfaces an error banner when reboot is unsupported / R48: surfaces an error banner when an operation is rejected (read-only) [^cap] |
...
| R64 | EventBridge ルール(バス選択セレクタ)一覧・作成(イベントパターン JSON)・有効/無効トグル・削除・ターゲット追加/削除(SQS キュー ARN)→ SDK 検証。トグルは `eventbridge.ruleState` でゲート(対称: kumo は InvalidAction をエラーバナー) | eventbridge.e2e.ts#R64: UI creates a rule and manages an SQS target / R64: UI toggles a rule enabled/disabled [^cap] / R64: surfaces an error when rule enable/disable is unsupported [^cap] |
...
| R75 | ECS クラスター: UI 作成 → 一覧表示(SDK で ACTIVE 検証)→ 名前確認で削除 + ダッシュボード(クラスター数 / サービス数 / 実行中タスク数)。制御プレーン非対応(localstack:3 Pro)なら `ecs-unsupported` バナーで作成非表示 | ecs.e2e.ts#R75: UI creates a cluster, lists it, then deletes it / R75: dashboard shows summary cards on a control-plane-capable emulator / R75: shows the ecs-unsupported banner when the control plane is unsupported [^cap] |
| R76 | ECS タスク定義: containerDefinitions JSON で登録 → SDK で DescribeTaskDefinition 裏取り → ファミリー:リビジョン一覧 → 行クリックでリビジョン詳細(コンテナ表示)→ 名前確認で登録解除。`ecs.taskDefinitions` でゲート(ListTaskDefinitions 非対応なら `ecs-unsupported` バナー) | ecs.e2e.ts#R76: registers a task definition, shows its detail, then deregisters it [^cap] / R76: shows the ecs-unsupported banner when task definitions are unlistable [^cap] |
| R77 | ECS クラスター詳細: サービスタブ(desiredCount で作成 → 別の希望数に変更し DescribeServices で裏取り → 名前確認削除、`ecs.services` ゲート)+ タスクタブ(RunTask 実行 → 一覧表示 → UI StopTask を DescribeTasks で裏取り、`ecs.runTask` 機能ゲート、軽量 busybox で必ず停止・後片付け)。kumo は `ecs-services-unsupported` / `ecs-tasks-unsupported` の対称ノートで検証 | ecs.e2e.ts#R77: creates a service, edits its desired count, then deletes it [^cap] / R77: runs a task and stops it [^cap] / R77: shows the services-unsupported note where ListServices is unsupported [^cap] / R77: shows the tasks-unsupported note where ListTasks is unsupported [^cap] |
| R78 | ECR リポジトリ一覧(名前 / URI / 作成日時)・作成・URI コピー・削除(名前入力確認、force オプション)。describe 非対応なら ecr-unsupported バナー + 作成ボタン非表示。describe 可・create 不可なら一覧描画 + 作成でエラーバナー。SDK 検証付き | ecr.e2e.ts#R78: UI creates a repository, copies its URI, then deletes it / R78: shows the unsupported banner and hides the create action / R78: lists repositories and surfaces an error when a create is rejected [^cap] |
| R79 | ECR リポジトリ詳細: イメージ一覧(タグ / ダイジェスト / サイズ / push 日時。push はスコープ外のため空表を表示)。describe 非対応なら ecr-unsupported バナー | ecr.e2e.ts#R79: shows an empty image list for a repository / R79: shows the unsupported banner on the detail page [^cap] |
| R80 | CloudWatch ロググループ: 一覧(名前 / 保持期間 / サイズ)・UI 作成 → SDK 検証・確認付き削除で行消滅 + ダッシュボード(ロググループ数 / アラーム数サマリ + クイックアクション) | cloudwatch.e2e.ts#R80: lists seeded log groups, creates and deletes via the UI, dashboard summarises |
| R81 | CloudWatch ロググループ詳細: ストリーム一覧 → ストリーム内イベント閲覧(SDK で seed したイベントが UI に出る)+ フィルタパターン検索(FilterLogEvents) | cloudwatch.e2e.ts#R81: shows a stream's seeded events and filters them |
| R82 | CloudWatch メトリクス: 名前空間 → メトリクス → 選択メトリクスの統計テーブル(GetMetricStatistics)。PutMetricData した値が反映。メトリクス非対応なら cloudwatch-unsupported バナー | cloudwatch.e2e.ts#R82: metrics namespace -> metric -> statistics reflect a seeded datapoint / R82: metrics page shows the unsupported banner where metrics are unavailable [^cap] |
| R83 | CloudWatch アラーム: 一覧(名前 / 状態 / メトリクス)・作成(メトリクス・しきい値・比較演算子)・確認付き削除。アラーム非対応なら cloudwatch-unsupported バナー | cloudwatch.e2e.ts#R83: creates and deletes an alarm via the UI / R83: alarms page shows the unsupported banner where alarms are unavailable [^cap] |
| R84 | Step Functions ステートマシン一覧(名前/タイプ/作成日時)+ ダッシュボード(ステートマシン数)。SDK seed を一覧表示・UI 作成(名前 + ASL JSON、ロールは dummy ARN 自動)→ SDK 検証・名前確認削除 → SDK 検証 | step-functions.e2e.ts#R84: lists a seeded state machine, creates + deletes via the UI |
| R85 | Step Functions 実行タブ: 実行開始(JSON 入力)→ 実行一覧 → 実行詳細(ステータス / 入力 / 出力 / イベント履歴テーブル)。Pass ステートで入力 `{"hello":"world"}` が出力に伝播することを検証(ポーリングで SUCCEEDED 待ち) | step-functions.e2e.ts#R85: starts an execution and the Pass output echoes the input |
| R86 | Step Functions 定義タブ: ASL 表示 + 更新(UpdateStateMachine → SDK 検証)。UpdateStateMachine 非対応エミュレータ(floci / kumo)では `sfn-update-unsupported` 案内文を表示 | step-functions.e2e.ts#R86: definition tab updates the ASL (supported) / R86: definition tab shows the unsupported notice [^cap] |
| R87 | OpenSearch ドメイン: UI 作成(名前)→ 一覧に行表示(SDK DescribeDomain で実在検証)→ 詳細でエンドポイント / ステータス / エンジンバージョン表示 → 名前確認付き削除で行消滅 | opensearch.e2e.ts#R87: UI creates a domain (SDK-verified), shows its detail, then deletes it [^cap] |
| R88 | OpenSearch cap ゲート: domains describe 非対応(kumo)なら opensearch-unsupported バナー + 作成非表示。describe 可・create 不可(socket 無し floci)なら一覧は通常描画し UI 作成でエラーバナー(RDS R35 と同型) | opensearch.e2e.ts#R88: shows the opensearch-unsupported banner and hides the create action / R88: lists domains but surfaces an error banner when a create is rejected [^cap] |
| R89 | Athena クエリエディタ: ステートメント入力 → 実行(StartQueryExecution → 状態ポーリング → GetQueryResults)→ 結果テーブル(非同期実行の「実行中...」状態あり)。結果値はエミュレータ依存のため「行が 1 行以上」で検証。`athena.query` 非対応(localstack:3 = Pro)なら `athena-unsupported` バナー | athena.e2e.ts#R89: runs SELECT 1 and renders at least one result row / R89: a SELECT on an Athena-less emulator shows the unsupported banner [^cap] |
| R90 | Athena ワークグループ CRUD: SDK seed をUI一覧で確認 + UI 作成(SDK 裏取り)+ 名前確認付き UI 削除。`athena.workgroups` 非対応(kumo)なら `athena-unsupported` バナー | athena.e2e.ts#R90: creates and deletes a workgroup (verified via the SDK) / R90: shows the unsupported banner on a workgroup-less emulator [^cap] |
| R91 | Athena 保存したクエリ: エディタから NamedQuery 保存(SDK 裏取り)→ 一覧 → エディタへ挿入(ステートメント復元)→ 名前確認付き削除。`athena.namedQueries` 非対応(floci/kumo/localstack)なら `athena-unsupported` バナー | athena.e2e.ts#R91: saves from the editor, recalls into the editor, and deletes / R91: shows the unsupported banner on a named-query-less emulator [^cap] |
| R92 | MSK(kafka 対応エミュレータ): UI でクラスター作成 → 一覧が `ACTIVE` 表示 → 詳細でブートストラップブローカー文字列表示 → UI 削除(SDK 裏取り)。BrokerNodeGroupInfo は最小構成(kafka.t3.small / subnet-1 / Kafka 3.6.0)を Rust 側でハードコード | msk.e2e.ts#R92: creates a cluster, shows its bootstrap brokers, then deletes it [^cap] |
| R93 | MSK(kafka 非対応エミュレータ = localstack Pro / kumo): `msk-unsupported` バナー表示 + 作成ボタン非表示(クラスターページ・ダッシュボードの対称テスト) | msk.e2e.ts#R93: shows the msk-unsupported banner and hides create on the clusters page / R93: shows the msk-unsupported banner on the dashboard [^cap] |
| R94 | SSM パラメータストア一覧(名前 / タイプ / バージョン)・作成(名前 `/path/` 形式・タイプ String\|StringList\|SecureString・値)・削除(名前入力確認)・パス prefix フィルタ入力(DescribeParameters の ParameterFilters BeginsWith) | ssm.e2e.ts#R94: UI creates String/StringList/SecureString params (SDK verified), prefix filter narrows the list, UI delete removes it |
| R95 | SSM パラメータ詳細: 値表示(SecureString は伏せ字 ⇄ 表示トグル `ssm-value-toggle`、WithDecryption)・値の更新(Overwrite → バージョン増加)は無条件。バージョン履歴テーブル(GetParameterHistory で v1/v2 が並ぶ)は `ssm.history` でゲート(対称: kumo は `ssm-history-unsupported` ノート) | ssm.e2e.ts#R95: masks a SecureString then reveals it, and overwrite bumps the version / R95: version history table lists v1 and v2 [^cap] / R95: shows the history-unsupported note where GetParameterHistory is unsupported [^cap] |
| R96 | Route 53 ホストゾーン: UI 作成 → 一覧(ドメイン名/タイプ/レコード数)→ 名前入力確認で削除(SDK 検証)+ ダッシュボード(ゾーン数 / ヘルスチェック数、非対応時は「-」) | route53.e2e.ts#R96: UI creates, lists and deletes a hosted zone / R96: dashboard summarises zones and the quick action opens the create modal |
| R97 | Route 53 ゾーン詳細: レコード一覧(NS/SOA 含む)・作成(名前・タイプ A/AAAA/CNAME/TXT/MX・TTL・複数行の値)・編集(UPSERT)・削除を各 SDK 検証 | route53.e2e.ts#R97: zone detail creates, edits (UPSERT) and deletes a record |
| R98 | Route 53 ヘルスチェック: 一覧・作成(IP/ドメイン・ポート・タイプ HTTP/TCP・パス)・確認付き削除。非対応エミュレータ(kumo は 404)では route53-unsupported バナー表示 + 作成ボタン非表示 | route53.e2e.ts#R98: UI creates and deletes a health check / R98: seeds a health check via the SDK and lists it in the UI / R98: health checks page shows the unsupported banner when not implemented [^cap] |
...
テストが最低 1 本は実行される。**2026-07-22 実測の capability マトリクス**:
    ...
    | rds.instances.stopStart (R48) | ○ | ×(UnsupportedOperation) | ○ | ×(CI 検証) |
    | rds.instances.modifyApplies (R48) | ○ | ×(受理するが storage 未適用) | ○ | ×(CI 検証) |
    | rds.snapshots.describe (R49) | × | ○ | ○ | × |
    | rds.snapshots.restore (R49) | × | × | ○ | × |
    | rds.parameterGroups.describe (R50) | × | ○ | ○ | × |
    | elasticache.describe (R68〜R70) | ×(Pro) | ○ | ○ | ○ |
    | opensearch.domains (R87/R88) | ○ | ○ | ○ | × |
    | opensearch.create (R87/R88) | ○ | △(docker.sock 必須) | ○ | × |
    | sqs.dlqSources (R38) | ○ | ○ | × | × |
    | sfn.updateStateMachine (R86) | ○ | ×(UnsupportedOperation) | ○ | ×(InvalidAction) |
    ...
    | lambda.layers (R55) | ×(ListLayers が 500 "list index out of range") | ○ | ○ | ×(ListLayers が NoSuchBucket/404) |
    ...
    | cloudformation.resourceReplacement (R74) | ×(UPDATE_COMPLETE だが置換未実行) | ○ | ○ | ×(CI 検証) |
    | ecs.clusters (R75〜R77) | ×(Pro 専用) | ○ | ○ | ○(制御プレーンのみ) |
    | ecs.taskDefinitions (R76) | ×(Pro 専用) | ○ | ○ | ×(ListTaskDefinitions が UnknownOperation) |
    | ecs.services (R77 サービス) | ×(Pro 専用) | ○ | ○ | ×(ListServices が UnknownOperation) |
    | ecs.tasks (R77 タスク一覧) | ×(Pro 専用) | ○ | ○ | ×(ListTasks が UnknownOperation) |
    | ecs.runTask (R77 タスク実行) | ×(Pro 専用) | ○ | ○ | ×(ListTasks 非対応・実タスク未生成) |
    | eventbridge.ruleState (R64 トグル) | ○ | ○ | ○ | ×(DisableRule が InvalidAction) |
    | ssm.history (R95 履歴) | ○ | ○ | ○ | ×(GetParameterHistory が「is not valid」) |
    | ecr.repositories (R78/R79) | ×(pro) | ○ | ○ | ○ |
    | ecr.create (R78/R79) | ×(pro) | ○(docker.sock マウント時) | ○ | ○ |
    | cloudwatch.metrics (R82) | ○(旧 Query プロトコル) | ○ | ○ | ×(monitoring 未ルーティング) |
    | cloudwatch.alarms (R83) | ○(旧 Query プロトコル) | ○ | ○ | ×(monitoring 未ルーティング) |
    | kafka.clusters (R92/R93) | ×(Pro) | ○ | ○(create 実測で対応) | ×(404) |
    | route53.healthChecks (R98) | ○ | ○ | ○ | ×(404 Not Found) |
    ...
    localstack・kumo がバナー。
    R87/R88 = ministack・localstack がドメインのフルライフサイクル(R87)、
    socket 無し floci が describe 可・create 不可の中間ケース(R88 エラーバナー)、
    kumo が unsupported バナー(R88)。R19 の SELECT 行描画は kumo のみスキップし、代わりに
    ...
    R75〜R77 = ministack・floci が ECS 制御プレーンフル(クラスター/タスク定義 CRUD、
    サービス、RunTask/StopTask)、localstack が `ecs-unsupported` バナー(Pro 専用)。
    kumo は制御プレーン部分対応(ListClusters/RegisterTaskDefinition/CreateService/RunTask は
    通るが ListTaskDefinitions/ListServices/ListTasks が UnknownOperation)なので、
    R76 は `ecs.taskDefinitions` でゲートし kumo は `ecs-unsupported` バナー側、
    R77 のサービス作成/希望数変更は `ecs.services`、タスク実行/停止は機能プローブ
    `ecs.runTask`(RunTask が ListTasks に現れる実タスクを生成する)でゲートし、
    kumo は `ecs-services-unsupported` / `ecs-tasks-unsupported` の対称ノートで検証する。
    R64 のルール有効/無効トグルは `eventbridge.ruleState` でゲートし、ministack・floci・
    localstack が SDK で DISABLED/ENABLED を裏取り、kumo は InvalidAction をエラーバナーに
    出す対称テストで検証する(R63 バス CRUD・R64 ルール/ターゲット CRUD・R65 配信は無条件)。
    R95 のバージョン履歴テーブルは `ssm.history` でゲートし、3 エミュレータが v1/v2 を並べ、
    kumo は `ssm-history-unsupported` ノート(値の伏せ字/更新は無条件)で検証する。
    R82/R83 は cloudwatch.metrics / cloudwatch.alarms でゲートし、localstack:3・floci・
    ministack がフル(Query プロトコル)、kumo が cloudwatch-unsupported バナー分岐となる。
