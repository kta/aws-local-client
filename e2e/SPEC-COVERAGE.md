...
| R76 | ECS タスク定義: containerDefinitions JSON で登録 → ファミリー:リビジョン一覧 → 行クリックでリビジョン詳細(コンテナ表示)→ 名前確認で登録解除。制御プレーン非対応なら `ecs-unsupported` バナー | ecs.e2e.ts#R76: registers a task definition, shows its detail, then deregisters it / R76: shows the ecs-unsupported banner on an unsupported emulator [^cap] |
| R77 | ECS クラスター詳細: サービスタブ(タスク定義 + desiredCount で作成・希望数変更・名前確認削除)+ タスクタブ(RunTask 実行 → 一覧表示 → StopTask、軽量 busybox イメージで必ず停止・後片付け) | ecs.e2e.ts#R77: creates a service with a desired count and edits it / R77: runs a task and stops it / R77: shows the services-unsupported note when ListServices is unsupported [^cap] |
...
| R95 | SSM パラメータ詳細: 値表示(SecureString は伏せ字 ⇄ 表示トグル `ssm-value-toggle`、WithDecryption)・値の更新(Overwrite → バージョン増加)・バージョン履歴テーブル(GetParameterHistory で v1/v2 が並ぶ) | ssm.e2e.ts#R95: masks a SecureString then reveals it, overwrites the value and the history table lists v1 and v2 |
...
テストが最低 1 本は実行される。**2026-07-22 実測の capability マトリクス**:
    ...
    kumo は制御プレーンのみ対応で RunTask が実コンテナを起動しないため、フル E2E は
    T17 に委譲する(ローカルは ministack + localstack の 2 種で検証)。
